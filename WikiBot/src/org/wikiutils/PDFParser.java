...
public class PDFParser {
    ...
    private static boolean createXMLFromPdf(String filename, String urls)
	    throws IOException {
	try {
    ...
    private static void process(String text, String filename, String urls)
	    throws IOException {
	...
	for (int i = 0; i < 30; i++) {
		...
		FileUtils.writeStringToFile(new File("shows/"
	...
	String[] brodcasters = { "CTV Total", "TSN+", "CBC Total",
		"Global Canada Com", "Global Canada", "City TV Total",
		"'A' Total", "Discovery SD+", "Global Com", "CTV Com",
		"Global Total" };
    ...
    private static void parseXML(String filename, String urls)
	    throws IOException {
	...
	for (int i = 0; i < 30; i++) {
	    ...
	    showName = showName.replace("/", "_backslash_").trim()
		    .replace("&apos;", "'").replace("&amp;", "&");
    ...
    /*
     * private static String getDateForURL(Calendar date) { String str =
     * date.get(Calendar.YEAR) + "/nat"; int month = date.get(Calendar.MONTH) +
     * 1; int day = date.get(Calendar.DATE); if (month < 10) str += "0" +
     * String.valueOf(month); else str += month; if (day < 10) str += "0" + day;
     * else str += day; str += date.get(Calendar.YEAR); return str; }
     */

    private static String formatViewers(String viewers, String ref) {
	float v = Float.parseFloat(viewers);
	v = (float) v / 1000;
	v = (float) Math.round(v * 100) / 100;

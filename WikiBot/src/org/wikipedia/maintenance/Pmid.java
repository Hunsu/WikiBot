...
public class Pmid {
	...
	try {
	    ...
	    String[] pages = wiki
		    .getCategoryMembers("Page_avec_une_référence_PMID_incomplète");
	    int size = pages.length;
	    for (int i = 0; i < size; i++) {
		String article = wiki.getPageText(pages[i], true);
		ArrayList<String> al = ParseUtils.getTemplates("cite pmid",
			article);
		int alSize = al.size();
		for (int j = 0; j < alSize; j++) {
		    try {
			int pmid = Integer.parseInt(ParseUtils
				.getTemplateParam(al.get(j), 1).trim());
			...
			if (enwiki.exists("Template:Cite pmid/" + pmid)) {
				...
				text = enal.get(0)
					...
					+ "</noinclude>";
				wiki.edit("Modèle:Cite pmid/" + pmid, text,
					"bot: création page");

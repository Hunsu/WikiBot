...
public class Wikification {
	...
	String category = "Saison de Médium";
	try {
	    boolean dosave = true;
	    boolean test = false;
	...
	for (String template : al) {
	    ...
	    String gimick = ParseUtils.getTemplateParam(template,
		    "nom du gimmick", false);
	    String value1 = ParseUtils.getTemplateParam(template,
		    "gimmickFlashBack", false);
	    String value2 = ParseUtils.getTemplateParam(template, "gimmick",
		    false);
	    if(value1 == null)
		continue;
	    if (gimick == null) {
		template = ParseUtils.renameTemplateParam(template,
			"nom du gimmickFlashBack", "nom du gimmick", false);
		template = ParseUtils.renameTemplateParam(template,
			"gimmickFlashBack", "gimmick", true);
		text = text.replace(oldTemplate, template);
		continue;
	    }
	    if (!gimick.trim().equals("") && value2 != null
		    && !value2.trim().equals("") && value1 != null && !value1.trim().equals("")) {
		template = ParseUtils.setTemplateParam(template,
			"nom du gimmick", " Immortel(s) et flashback\n", false);
		template = ParseUtils.removeTemplateParam(template,
			"nom du gimmickFlashBack");
		template = ParseUtils.removeTemplateParam(template,
			"gimmickFlashBack");
		value2 = "\n;Immortels" + value2 + ";FlashBack\n" + value1+"\n";
		value2 = value2.replace("\n*", "\n:*").replace("\n\n", "\n");
		template = ParseUtils.setTemplateParam(template, "gimmick", value2, true);
		text = text.replace(oldTemplate, template);
	    } else {
		template = ParseUtils.removeTemplateParam(oldTemplate, "gimmick");
		template = ParseUtils.removeTemplateParam(template, "nom du gimmick");
		template = ParseUtils.renameTemplateParam(template, "nom du gimmickFlashBack", "nom du gimmick", false);
		template = ParseUtils.renameTemplateParam(template, "gimmickFlashBack", "gimmick", true);
		text = text.replace(oldTemplate, template);
	    }
	...
	String resPattern = "(?i)\\*\\s*'''Résumé'''\\s*:|\\*'''résumé''' :|\\*''' Résumé''' :|\\*'''Synopsis''' :|\\*\\s*Synopsis\\s*:|\\*'''Résumé''' :|\\* '''Résumé :''' ";
	String refPattern = "(?i)\\*\\s*'''Références'''\\s*:|\\*\\s*'''Clin d’œil'''\\s*:";
	String comPattern = "(?i)\\*\\s*'''(Commentaires?\\(?s?\\)?)?(Remarques?)?'''\\s*:|\\* '''Note''' :|\\*\\s*Note :|\\*\\s*'''Notes'''\\s*:|\\*\\s*'''commentaire'''\\s*:|\\* '''Remarques? :'''";
	...
	s = RegExReplace(s,
		...
		"| autre titre     = $1");
	s = RegExReplace(s, "(?i)Titre original\\s*:?\\s*''(.*)''",
		"{{Saison de série télévisée/Épisode\n| titre original     = $1");
	s = RegExReplace(s, "(?i)Titre québécois\\s*:?\\s*''(.*)''",

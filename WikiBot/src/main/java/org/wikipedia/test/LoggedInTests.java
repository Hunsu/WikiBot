...
public class LoggedInTests {
		...
		try {
			...
			while ((strLine = br.readLine()) != null) {
				try {
					...
					int nb = al.size();
					for (int k = 0; k < nb; k++) {
						text = text.replace(al.get(k),
								citeWeb(al.get(k), errors));
					}
	...
	public static String citeWeb(String template, HashMap<String, String> map) {
		for (String key : map.keySet()) {
			...
			if (value.trim().equals("sup"))
				...
				template = ParseUtils.renameTemplateParam(template, key, value,
						false);
		}
		try {
		...
		if (date != null) {
			...
			if (m.find()) {
				if (Integer.parseInt(m.group(2)) < 13) {
					...
					template = ParseUtils.setTemplateParam(template, param,
							newDate, false);
				}
			}
		}
	...
	/*
	 * public static String renameParameters(String template){ template =
	 * ParseUtils.renameTemplateParam(template, "mis en ligne le",
	 * "date",false); template = ParseUtils.renameTemplateParam(template,
	 * "consulte le", "consulté le",false); template =
	 * ParseUtils.renameTemplateParam(template, "Date", "date",false); template
	 * = ParseUtils.renameTemplateParam(template, "Auteur", "auteur",false);
	 * template = ParseUtils.renameTemplateParam(template, "publié le",
	 * "date",false); template = ParseUtils.renameTemplateParam(template,
	 * "accédé le", "consulté le",false); template =
	 * ParseUtils.renameTemplateParam(template, "date consultation",
	 * "consulté le",false); template = ParseUtils.renameTemplateParam(template,
	 * "éditeurr", "éditeur",false); template =
	 * ParseUtils.renameTemplateParam(template, "editor", "éditeur",false);
	 * template = ParseUtils.renameTemplateParam(template, "editor",
	 * "éditeur",false); template = ParseUtils.renameTemplateParam(template,
	 * "autor", "auteur",false); template =
	 * ParseUtils.renameTemplateParam(template, "consultéle",
	 * "consulté le",fal); template = ParseUtils.renameTemplateParam(template,
	 * "date d'accès", "date"); template =
	 * ParseUtils.renameTemplateParam(template, "mise en ligne le", "date");
	 * template = ParseUtils.renameTemplateParam(template, "accédé le",
	 * "consulté le"); template = ParseUtils.renameTemplateParam(template,
	 * "date consultation", "consulté le"); template =
	 * ParseUtils.renameTemplateParam(template, "Éditeur", "éditeur"); template
	 * = ParseUtils.renameTemplateParam(template, "date de consultation",
	 * "consulté le"); template = ParseUtils.renameTemplateParam(template,
	 * "onsulté le", "consulté le"); template =
	 * ParseUtils.renameTemplateParam(template, "visité le", "consulté le");
	 * template = ParseUtils.renameTemplateParam(template, "Editeur",
	 * "éditeur"); template = ParseUtils.renameTemplateParam(template,
	 * "contulté le", "consulté le"); template =
	 * ParseUtils.renameTemplateParam(template, "site web", "site"); template =
	 * ParseUtils.renameTemplateParam(template, "année accès", "année"); return
	 * template; }
	 */
}

...
public class ParseUtils {
     ...
     * @return (see above) Contributed by Hunsu
     */
    ...
    /**
     * Removes the given template parameter from the input string. Example:
     * removeTemplateParam("{{template|X=A|Y=B|Z=C}}", "Y") ==
     * {{template|X=A|Z=C}}.
     *
     * @param template
     *            the input string
     ...
     * @return the input string with the given parameter removed Contributed by
     *         Hunsu
     */
     ...
     * @return the result string Contributed by Hunsu.
     */
     ...
     * @param position
     *            the position
     * @return true, if the position will be ignored Contributed by Hunsu.
     */
     ...
     * @param end
     *            the ending string
     * @return the start and end of text contanied between no Contributed by
     *         Hunsu.
     */
     ...
     * @param text
     *            the wikitext
     * @return the internal links Contributed by Hunsu.
     */
     ...
     * @param adjust
     *            if we should pad the parameter length with spaces
     * @return the new template wikitext Contributed by Hunsu.
     */
     ...
     * @param map
     *            the map
     * @return the template wikitext e.g. {{example|1=Blah|2=Blah2}} Contributed
     *         by Hunsu.
     */
     ...
     *            the new name
     * @param adjust
     *            if we should adjust the parameters length (see
     *            {@link #adjust(java.lang.String, int) }).
     * @return the new template Contributed by Hunsu.
     */
     ...
     *
     * @param map
     *            the map that contains the template parameters
     * @return the new map
     * @see #getTemplateParameters Contributed by Hunsu.
     */
	...
	for (String key : map.keySet()) {
		...
		newMap.put(adjust(key, length + 1), map.get(key));
	}
	return newMap;
    }
     ...
     *
     * @param map
     *            a map containing template parameters
     * @param param
     *            the param to look for
     * @param trim
     *            if we should trim the result
     * @return the template param Contributed by Hunsu.
     */
     ...
     *            wikitext that consists of only a template call e.g.
     *            {{example|1=Blah|2=Blah2}}
     * @return the template name, here "example" Contributed by Hunsu.
     */
    ...
    public static HashMap<String, String> getRefs(String text) {
	HashMap<String, String> map = new HashMap<String, String>();
	Pattern p = Pattern.compile("(<ref.*?>)(.*?)</ref>");
	Pattern p2 = Pattern.compile("<ref.*?/>");
	Matcher m = p.matcher(text);
	int i = 1;
	while (m.find()) {
	    String name = getName(m.group(1));
	    if (name == null) {
		map.put(String.valueOf(i), m.group());
		i++;
	    } else
    ...
    private static String getName(String ref) {
	int index = ref.indexOf("name");
	if (index == -1)
	    return null;
	index = ref.indexOf("=", index) + 1;
	int i = ref.indexOf("\"", index);
	if (i != -1) {
	    index = i + 1;
	    i = ref.indexOf("\"", index);
	} else {
    ...
    public static String renameTemplate(String template, String name) {
	LinkedHashMap<String, String> params = getTemplateParametersWithValue(template);
	params.put("templateName", name);
	return templateFromMap(params);
    }

    public static String formatTemplate(String template,
	    List<String> paramsOrder, String paramFormat) {
	String[] format = paramFormat.split("=");
	for (String key : paramsOrder) {
	    String value = ParseUtils.getTemplateParam(template, key, false);
	    if (value == null)
		continue;
	    template = ParseUtils.removeTemplateParam(template, key);
	    template = ParseUtils.setTemplateParam(template,
		    key,
		    format[1].replace("value", value.trim()), false);
	}
